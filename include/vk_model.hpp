...
class ModelData {
    ...
    void loadImages(const tinygltf::Model& model, vk::Device device, const Allocator& allocator, CommandBuffer& cmdBuf, vk::Queue queue) {
        for (const auto& image : model.images) {
            ...
            textureImages.emplace_back(device, allocator, cmdBuf, queue, extent,
                std::span<const std::byte>{ reinterpret_cast<const std::byte*>(image.image.data()), image.image.size() });
        }
        ...
        if (!defaultTexture) {
            ...
            const std::byte dat[4] = {};

            defaultTexture.emplace(device, allocator, cmdBuf, queue, extent, std::span<const std::byte>(dat));
        }
    }
