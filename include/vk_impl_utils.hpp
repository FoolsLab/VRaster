...
public:
    ...
    TextureImage(vk::Device device, const Allocator& allocator, CommandBuffer& cmdBuf, vk::Queue queue, vk::Extent3D extent, const std::span<const std::byte> imgData, vk::SharingMode share = vk::SharingMode::eExclusive)
    {
        vk::DeviceSize size = imgData.size_bytes();
        Buffer staging{ device, allocator, size, vk::BufferUsageFlagBits::eTransferSrc, vk::MemoryPropertyFlagBits::eHostVisible };
...
public:
    RenderProc(const vk::Device _device, const vk::Format _format)
        ...
        vertShader(device, "assets/shader.vert.spv"), fragShader(device, "assets/shader.frag.spv")
    {
        CreateRenderpass();
